...
public:
    ...
    int saveMeshZip(string fileName, string archiveName) {
       ...
       int openMeshZip(string archiveFileName, string archiveFolder){
        ...
        mz_zip_archive zip_archive; 
        memset(&zip_archive, 0, sizeof(zip_archive)); //Saving memory for the zip archive
        retVal = mz_zip_reader_init_file(&zip_archive, archiveFileName.c_str(),
                                         0); // Initializing the zip file reader
        if (retVal == MZ_FALSE) {
          printf("mz_zip_reader_init_file returned with error\n");
          return -1;
        }
        ...
        for (int i = 0; i < (int)mz_zip_reader_get_num_files(&zip_archive); i++){
            ...
            retVal = mz_zip_reader_file_stat(&zip_archive, i, &file_stat);
            if (retVal == MZ_FALSE) {
                printf("mz_zip_reader_file_stat returned with error\n");
                return -1;
            }
...
//               mz_zip_reader_extract_file_to_file(&zip_archive, file_stat.m_filename, dest.c_str(), 0);
               ...
               printf("Extracting to2: %s, \n",  file_stat.m_filename);
               retVal = mz_zip_reader_extract_file_to_file(&zip_archive, file_stat.m_filename, file_stat.m_filename, 0);
               if (retVal == MZ_FALSE) {
                   printf("mz_zip_reader_extract_file_to_file returned with error1\n");
                   return -1;
               }
        ...
        if(meshIndex > -1){
            mz_zip_archive_file_stat file_stat; //Get the info about each file and store them into file_stat
            retVal = mz_zip_reader_file_stat(&zip_archive, meshIndex, &file_stat);    
            if (retVal == MZ_FALSE) {
                printf("mz_zip_reader_file_stat returned with error\n");
                return -1;
            }
            ...
            if( retVal == MZ_FALSE )
            {
                printf("mz_zip_reader_extract_file_to_file returned with error2\n");
                return -1;
            }
            
            printf("meshFileName: %s \n",  meshFileName.c_str());

            // I open the mesh, which will have texture support (WedgeTextureCoord) from which I can take the list of textures that I use
        ...
        retVal = mz_zip_reader_end(&zip_archive);    //Close the zip file
        if (retVal == MZ_FALSE) {
            printf("mz_zip_reader_end returned with error\n");
            return -1;
        }
...
EMSCRIPTEN_BINDINGS(CppMesh) {
    emscripten::class_<CppMesh>("CppMesh")
            ...
            .function("getMaterialName",        &CppMesh::getMaterialName)
            ;
